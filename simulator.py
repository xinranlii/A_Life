...
def create_spider_model(model_file, random_num_upper_legs, random_num_lower_legs, u_mass, l_mass, gear, u_joint_range_min, l_joint_range_min):
    ...
    for i in range(1, 5):  # Positions 1 to 4
        upper_leg_node = ParentNode(main_body, i, 0, 0, 'upper_leg', actuators).create_upper_leg(f"leg{i}", leg_colors[i-1], u_mass, gear, u_joint_range_min)
        ...
        for q in range(int(random_num_upper_legs)):
            child = parent.create_upper_leg(f"leg{4*(q+1)+i}", leg_colors[i-1], u_mass, gear, u_joint_range_min)
            parent = child
        for j in range(int(random_num_lower_legs)):
            child = parent.create_lower_leg(f"leg{4*j+i}", "0.486 0.917 0.97 0.75",l_mass, gear, l_joint_range_min)
            parent = child
