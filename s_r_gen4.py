...
def create_spider_model(model_file, random_num_upper_legs, random_num_lower_legs):
    ...
    u_gear = mutated_variables['new_upper_gear']
    l_gear = mutated_variables['new_lower_gear']
    ...
    for i in range(1, 5):  # Positions 1 to 4
        upper_leg_node = ParentNode(main_body, i, 0, 0, 'upper_leg', actuators).create_upper_leg(f"leg{i}", leg_colors[i-1], u_mass, u_gear, (u_joint_range_min, u_joint_range_max))
        ...
        for q in range(random_num_upper_legs):
            child = parent.create_upper_leg(f"leg{4*(q+1)+i}", leg_colors[i-1], u_mass, u_gear, (u_joint_range_min, u_joint_range_max))
            parent = child
        for j in range(random_num_lower_legs):
            child = parent.create_lower_leg(f"leg{4*j+i}", "0.486 0.917 0.97 0.75",l_mass, l_gear, (l_joint_range_min, l_joint_range_max))
            parent = child
...
def fitness_function(model_file, control_strategy, randomness, simulation_steps=1000):
    ...
    stability_metric = 0  
    fitness_score = distance_moved - stability_metric 

    return fitness_score

def calculate_fitness(distance, energy, stability, weights=(0.5, -0.2, 0.3)):
    """Weights: tuple of weights for distance, energy, and stability respectively."""
    return weights[0] * distance + weights[1] * energy + weights[2] * stability
...
def randomness():
    random_num_upper_legs = random.randint(0, 2)
    random_num_lower_legs = random.randint(random_num_upper_legs, 5)
    print(random_num_upper_legs + 1, "layers of upper legs")
    print(random_num_lower_legs, "layers of lower legs")
    return random_num_upper_legs, random_num_lower_legs
...
def mutation():
    ...
    mutated_variables = {
        'new_upper_mass': random.uniform(*mass_upper_mutation_range),
        'new_lower_mass': random.uniform(*mass_lower_mutation_range),
        'new_upper_gear': random.uniform(*gear_mutation_range),
        'new_lower_gear': random.uniform(*gear_mutation_range),
...
def display_the_best(best_row):
    ...
    u_gear = best_row['new_upper_gear']  
    l_gear = best_row['new_lower_gear']  
    ...
    sim2.create_spider_model("best_fitness_model.xml", str(random_num_upper_legs), str(random_num_lower_legs), str(u_mass), str(l_mass), str(u_gear), str(l_gear), u_joint_range, l_joint_range)
...
best_row = find_best_settings('gen4.csv')
